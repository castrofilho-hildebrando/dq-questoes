...
export default function EncontrosAoVivo() {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { goBack, fromSuffix } = useBackNavigation();

  const { data: sections = [], isLoading: loadingSections } = useQuery({
    queryKey: ["gravacao-sections"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("gravacao_sections")
        .select("*")
        .eq("is_active", true)
        .order("display_order");
  ...
  const { data: progress = [] } = useQuery({
    queryKey: ["gravacao-progress", user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      const { data, error } = await supabase
      ...
      <div className="min-h-screen bg-background">
        <header className="sticky top-0 z-30 border-b border-border/50 bg-background/80 backdrop-blur-md">
          <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-foreground">Encontros ao Vivo — Gravações</h1>
          </div>
        </header>
        ...
        <div className="relative py-10 px-6 overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl mx-auto text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-2">Gravações</h2>
            <p className="text-muted-foreground text-base md:text-lg">
              Assista às gravações dos encontros ao vivo e acompanhe seu progresso.
            </p>
          </div>
        </div>
        ...
        <main className="max-w-7xl mx-auto px-4 py-6 space-y-10">
          {sections.length === 0 && (
            <p className="text-center text-muted-foreground py-20">
              Nenhuma gravação disponível ainda. Volte em breve!
            </p>
          )}
                  ...
                  <div className="flex gap-4 pb-4">
                    {sectionModules.map((mod) => (
                      <ModuleCard
                        ...
                        onClick={() => navigate(`/encontros-ao-vivo/${mod.id}${fromSuffix}`)}
                      />
                    ))}
                  </div>
