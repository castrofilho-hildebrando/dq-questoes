...
export default function DidaticaPlayer() {
  ...
  const { data: mod, isLoading } = useQuery({
    queryKey: ["didatica-module", moduleId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("didatica_modules")
        .select("*, didatica_sections(title)")
        .eq("id", moduleId!)
        .single();
  ...
  const { data: progressRow } = useQuery({
    queryKey: ["didatica-progress-single", user?.id, moduleId],
    queryFn: async () => {
      if (!user?.id || !moduleId) return null;
      const { data } = await supabase
        .from("didatica_progress")
        .select("*")
        .eq("user_id", user.id)
  ...
  const markComplete = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("didatica_progress").upsert(
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["didatica-progress"] });
      queryClient.invalidateQueries({ queryKey: ["didatica-progress-single"] });
    },
  });
  ...
  if (!mod) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate("/didatica")}>Voltar</Button>
      </div>
    );
  }
  ...
  const sectionTitle = mod.didatica_sections?.title;
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-30 border-b border-border/50 bg-background/80 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate("/didatica")}>
