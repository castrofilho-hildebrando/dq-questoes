...
export default function DidaticaHome() {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { goBack, fromSuffix } = useBackNavigation();

  const { data: sections = [], isLoading: loadingSections } = useQuery({
    queryKey: ["didatica-sections"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("didatica_sections")
        .select("*")
        .eq("is_active", true)
        .order("display_order");
  ...
  const { data: progress = [] } = useQuery({
    queryKey: ["didatica-progress", user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      const { data, error } = await supabase
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-30 border-b border-border/50 bg-background/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-foreground">Dissecando a Didática</h1>
        </div>
      </header>
      ...
      <div className="relative py-10 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          ...
          <p className="text-muted-foreground text-base md:text-lg">
            Aperfeiçoe sua didática com métodos comprovados. Assista, estude e acompanhe seu progresso.
          </p>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-10">
        <TutorialActionCard productSlug="dissecando_didatica" />
        {sections.length === 0 && (
                ...
                <div className="flex gap-4 pb-4">
                  {sectionModules.map((mod) => (
                    <ModuleCard
                      ...
                      onClick={() => navigate(`/didatica/${mod.id}${fromSuffix}`)}
                    />
                  ))}
                </div>
