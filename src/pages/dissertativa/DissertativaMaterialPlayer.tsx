...
export default function DissertativaMaterialPlayer() {
  ...
  // Fetch profile for watermark and 7-day restriction
  ...
  const { data: mod, isLoading } = useQuery({
    queryKey: ["dissertativa-material-module", moduleId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("dissertativa_material_modules")
        .select("*, dissertativa_material_sections(title)")
        .eq("id", moduleId!)
        .single();
  ...
  const { data: progressRow } = useQuery({
    queryKey: ["dissertativa-material-progress-single", user?.id, moduleId],
    queryFn: async () => {
  ...
  const markComplete = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("dissertativa_material_progress").upsert(
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["dissertativa-material-progress"] });
      queryClient.invalidateQueries({ queryKey: ["dissertativa-material-progress-single"] });
    },
  });
  ...
  if (!mod) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate(`/dissertativa/${courseId}/materiais${fromSuffix}`)}>
          Voltar
        </Button>
      </div>
    );
  ...
  const sectionTitle = mod.dissertativa_material_sections?.title;

  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-30 border-b border-border/50 bg-background/80 backdrop-blur-md">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate(`/dissertativa/${courseId}/materiais${fromSuffix}`)}>
