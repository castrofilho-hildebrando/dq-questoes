...
export default function TutoriaisPlayer() {
  ...
  const { data: mod, isLoading } = useQuery({
    queryKey: ["tutorial-module", moduleId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("tutorial_modules")
        .select("*, tutorial_sections(title)")
        .eq("id", moduleId!)
        .single();
  ...
  const { data: progressRow } = useQuery({
    queryKey: ["tutorial-progress-single", user?.id, moduleId],
    queryFn: async () => {
  ...
  const markComplete = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["tutorial-progress-single"] });
    },
  });
  ...
  if (!mod) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4">
        ...
        <Button variant="outline" onClick={() => navigate("/tutoriais")}>Voltar</Button>
      </div>
    );
  }
  ...
  const sectionTitle = mod.tutorial_sections?.title;
      ...
      <div className="min-h-screen bg-background">
        <header className="sticky top-0 z-30 border-b border-border/50 bg-background/80 backdrop-blur-md">
          <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={() => navigate("/tutoriais?from=dossie-if")}>
